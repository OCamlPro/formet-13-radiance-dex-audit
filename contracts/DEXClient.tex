...
\begin{itemize}
\item \issueError{}
\item Minor Issue: {\tt counterCallback} should probably be
...
\begin{itemize}
\item Minor Issue: there is probably no need to spend gas with {\tt
  tvm.accept} since the method can be executed locally (get-method).
\item Minor Issue: the method should probably fail with {\tt require}
  if the callback id does not exist.
\end{itemize}
...
\item \issueAccept{DEXClient.tokensReceivedCallback}{
  ...
  \item An attacker could send many such messages also to include
    ...
    correct ones by sending more than 10 such messages.
...
\begin{itemize}
\item Minor Issue: This internal function is unused.
\item \issueInternal
\end{itemize}
